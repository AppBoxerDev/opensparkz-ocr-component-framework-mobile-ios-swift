...
open class ValidationTextField: UITextField {
    ...
    @IBInspectable
    open var titleColor: UIColor = .clear {
        didSet{
            update()
        }
    }
    ...
    public var validateStatus: ValidateType = .ready {
        didSet {
    ...
    public  func initValidationTextField() {
        borderStyle = .none
        createTitleLabel()
        createLineView()
        createErrorLabel()
    ...
    private func updateTitleVisibility(_ animated: Bool = false) {
        ...
        let errorAlpha: CGFloat = isValid || text == "" ? 1.0 : 1.0
    ...
    override open func canPerformAction(_ action: Selector, withSender sender: Any?) -> Bool {
        UIMenuController.shared.isMenuVisible = false //do not display the menu
        // resignFirstResponder() //do not allow the user to selected anything
        return false
    }
